...
void MACRIFY(oqs_kex_lwe_frodo_round2)(unsigned char *out, uint16_t *in) {
	oqs_kex_lwe_frodo_key_round(in, PARAMS_NBAR * PARAMS_NBAR,
	                            PARAMS_LOG2Q - PARAMS_EXTRACTED_BITS);
	for (int i = 0; i < PARAMS_NBAR * PARAMS_NBAR; i++) {
		in[i] >>=
		    PARAMS_LOG2Q - PARAMS_EXTRACTED_BITS; // drop bits that were zeroed out
	}

	// out should have enough space for the key
	oqs_kex_lwe_frodo_pack(out, PARAMS_KEY_BITS / 8, in,
	                       PARAMS_NBAR * PARAMS_NBAR, PARAMS_EXTRACTED_BITS);
}

void MACRIFY(oqs_kex_lwe_frodo_crossround2)(unsigned char *out,
                                            const uint16_t *in) {
...
void MACRIFY(oqs_kex_lwe_frodo_reconcile)(unsigned char *out, uint16_t *w,
                                          const unsigned char *hint) {
	oqs_kex_lwe_frodo_key_round_hints(w, PARAMS_NBAR * PARAMS_NBAR,
	                                  PARAMS_LOG2Q - PARAMS_EXTRACTED_BITS, hint);
	for (int i = 0; i < PARAMS_NBAR * PARAMS_NBAR; i++) {
		w[i] >>=
		    PARAMS_LOG2Q - PARAMS_EXTRACTED_BITS; // drop bits that were zeroed out
	}
	oqs_kex_lwe_frodo_pack(out, PARAMS_KEY_BITS / 8, w, PARAMS_NBAR * PARAMS_NBAR,
	                       PARAMS_EXTRACTED_BITS);
}

// Generate-and-multiply: generate A row-wise, multiply by s on the right.
void MACRIFY(oqs_kex_lwe_frodo_mul_add_as_plus_e_on_the_fly)(
    uint16_t *out, const uint16_t *s, const uint16_t *e,
	...
	for (int i = 0; i < PARAMS_N; i++) {
		...
		OQS_AES128_ECB_enc_sch((uint8_t *) a_row, sizeof(a_row), aes_key_schedule,
		                       (uint8_t *) a_row);
...
void MACRIFY(oqs_kex_lwe_frodo_mul_add_sa_plus_e_on_the_fly)(
    uint16_t *out, const uint16_t *s, const uint16_t *e,
    struct oqs_kex_lwe_frodo_params *params) {
	// a (N x N)
	// s',e' (N_BAR x N)
	...
	for (int kk = 0; kk < PARAMS_N; kk += PARAMS_STRIPE_STEP) {
		...
		OQS_AES128_ECB_enc_sch((uint8_t *) a_cols, sizeof(a_cols), aes_key_schedule,
		                       (uint8_t *) a_cols);
		...
		for (int i = 0; i < PARAMS_NBAR; i++) {
			for (int k = 0; k < PARAMS_STRIPE_STEP; k++) {
				...
				out[i * PARAMS_N + kk + k] &=
				    PARAMS_Q - 1; // Works as long as PARAMS_Q is a power of 2
			}
		}
	}
	OQS_AES128_free_schedule(aes_key_schedule);
...
void MACRIFY(oqs_kex_lwe_frodo_mul_bs)(uint16_t *out, const uint16_t *b,
                                       const uint16_t *s) {
	// b (N_BAR x N)
	// s (N x N_BAR)
	// out = bs
...
void MACRIFY(oqs_kex_lwe_frodo_mul_add_sb_plus_e)(uint16_t *out,
                                                  const uint16_t *b,
                                                  const uint16_t *s,
                                                  const uint16_t *e) {
	// b (N x N_BAR)
	...
	for (int k = 0; k < PARAMS_NBAR; k++) {
		for (int i = 0; i < PARAMS_NBAR; i++) {
			...
			out[k * PARAMS_NBAR + i] &=
			    PARAMS_Q - 1; // not really necessary since LWE_Q is a power of 2.
		}
	}
}
