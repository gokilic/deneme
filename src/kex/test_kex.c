...
static OQS_STATUS
kex_test_correctness_wrapper(OQS_RAND* rand, enum OQS_KEX_alg_name alg_name,
                             const uint8_t* seed, const size_t seed_len,
                             const char* named_parameters, int iterations,
                             bool quiet)
{
    OQS_KEX* kex = NULL;
    OQS_STATUS ret;
    ...
    {
        ret = kex_test_correctness(rand, alg_name, seed, seed_len,
                                   named_parameters, 0, occurrences);
        if (ret != OQS_SUCCESS)
        {
            goto err;
        }
...
static OQS_STATUS
kex_bench_wrapper(OQS_RAND* rand, enum OQS_KEX_alg_name alg_name,
                  const uint8_t* seed, const size_t seed_len,
                  const char* named_parameters, const size_t seconds)
{
    ...
    TIME_OPERATION_SECONDS({
                               OQS_KEX_alice_0(kex, &alice_priv, &alice_msg,
                                               &alice_msg_len);
    ...
    TIME_OPERATION_SECONDS({
                               OQS_KEX_bob(kex, alice_msg, alice_msg_len,
                                           &bob_msg, &bob_msg_len, &bob_key,
                                           &bob_key_len);
    ...
    OQS_KEX_bob(kex, alice_msg, alice_msg_len, &bob_msg, &bob_msg_len, &bob_key,
                &bob_key_len);
    TIME_OPERATION_SECONDS({
                               OQS_KEX_alice_1(kex, alice_priv, bob_msg,
                                               bob_msg_len, &alice_key,
                                               &alice_key_len);
                               free(alice_key);
                           }, "alice 1", seconds);
    alice_key = NULL;
...
static OQS_STATUS
kex_mem_bench_wrapper(OQS_RAND* rand, enum OQS_KEX_alg_name alg_name,
                      const uint8_t* seed, const size_t seed_len,
                      const char* named_parameters)
{
...
int main(int argc, char** argv)
{
    ...
    if (mem_bench)
    {
        for (size_t i = 0; i < kex_testcases_len; i++)
        {
            ...
            {
                success = kex_mem_bench_wrapper(rand, kex_testcases[i].alg_name,
                                                kex_testcases[i].seed,
                                                kex_testcases[i].seed_len,
                                                kex_testcases[i].named_parameters);
            }
    ...
    for (size_t i = 0; i < kex_testcases_len; i++)
    {
        if (run_all || kex_testcases[i].run == 1)
        {
            ...
            success = kex_test_correctness_wrapper(rand,
                                                   kex_testcases[i].alg_name,
                                                   kex_testcases[i].seed,
                                                   kex_testcases[i].seed_len,
                                                   kex_testcases[i].named_parameters,
                                                   num_iter, quiet);
