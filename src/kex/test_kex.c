...
struct kex_testcase kex_testcases[] = {
    ...
    {OQS_KEX_alg_lwe_frodo, (unsigned char *) "01234567890123456", 16,
     "recommended", "lwe_frodo_recommended", 0, 100},
#endif
    ...
    {OQS_KEX_alg_sidh_cln16_compressed, NULL, 0, NULL, "sidh_cln16_compressed",
     0, 10},
#endif
...
static OQS_STATUS
kex_test_correctness(OQS_RAND *rand, enum OQS_KEX_alg_name alg_name,
                     const uint8_t *seed, const size_t seed_len,
                     const char *named_parameters, const int print,
	...
	/* Bob's response */
	rc = OQS_KEX_bob(kex, alice_msg, alice_msg_len, &bob_msg, &bob_msg_len,
	                 &bob_key, &bob_key_len);
	if (rc != OQS_SUCCESS) {
	...
	rc = OQS_KEX_alice_1(kex, alice_priv, bob_msg, bob_msg_len, &alice_key,
	                     &alice_key_len);
	if (rc != OQS_SUCCESS) {
	...
	if (alice_key_len != bob_key_len) {
		eprintf("ERROR: Alice's session key and Bob's session key are different "
		        "lengths (%zu vs %zu)\n",
...
static OQS_STATUS kex_test_correctness_wrapper(OQS_RAND *rand,
                                               enum OQS_KEX_alg_name alg_name,
                                               const uint8_t *seed,
	...
	ret = kex_test_correctness(rand, alg_name, seed, seed_len, named_parameters,
	                           quiet ? 0 : 1, occurrences);
	...
	printf("====================================================================="
	       "===========\n");
	printf("Testing correctness and randomness of key exchange method %s "
	       "(params=%s) for %d iterations\n",
...
static void cleanup_alice_0(OQS_KEX *kex, void *alice_priv,
                            uint8_t *alice_msg) {
	free(alice_msg);
	OQS_KEX_alice_priv_free(kex, alice_priv);
}
...
static OQS_STATUS kex_bench_wrapper(OQS_RAND *rand,
                                    enum OQS_KEX_alg_name alg_name,
                                    const uint8_t *seed, const size_t seed_len,
                                    const char *named_parameters,
	...
	TIME_OPERATION_SECONDS({
		...
		cleanup_alice_0(kex, alice_priv, alice_msg);
	},
	                       "alice 0", seconds);

	OQS_KEX_alice_0(kex, &alice_priv, &alice_msg, &alice_msg_len);
	TIME_OPERATION_SECONDS({
		OQS_KEX_bob(kex, alice_msg, alice_msg_len, &bob_msg, &bob_msg_len, &bob_key,
		            &bob_key_len);
	...
	OQS_KEX_bob(kex, alice_msg, alice_msg_len, &bob_msg, &bob_msg_len, &bob_key,
	            &bob_key_len);
	TIME_OPERATION_SECONDS({
		OQS_KEX_alice_1(kex, alice_priv, bob_msg, bob_msg_len, &alice_key,
		                &alice_key_len);
		free(alice_key);
	},
	                       "alice 1", seconds);
	...
	printf("Communication (bytes): A->B: %zu, B->A: %zu, total: %zu; "
	       "classical/quantum security bits [%u:%u] \n",
	       alice_msg_len, bob_msg_len, alice_msg_len + bob_msg_len,
	       kex->estimated_classical_security, kex->estimated_quantum_security);
...
static OQS_STATUS kex_mem_bench_wrapper(OQS_RAND *rand,
                                        enum OQS_KEX_alg_name alg_name,
                                        const uint8_t *seed,
                                        const size_t seed_len,
                                        const char *named_parameters) {
...
void print_help() {
	...
	printf("  --mem-bench\n");
	printf("    Run memory benchmarks (run once and allocate only what is "
	       "required)\n");
	printf("\nalgorithms:\n");
	size_t kex_testcases_len =
...
int main(int argc, char **argv) {
	...
	for (int i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			...
			} else if ((strcmp(argv[i], "--mem-bench") == 0 ||
			            strcmp(argv[i], "-m") == 0)) {
				mem_bench = true;
			}
		} else {
	...
	if (mem_bench) {
		for (size_t i = 0; i < kex_testcases_len; i++) {
			if (run_all || kex_testcases[i].run == 1) {
				success = kex_mem_bench_wrapper(
				    rand, kex_testcases[i].alg_name, kex_testcases[i].seed,
				    kex_testcases[i].seed_len, kex_testcases[i].named_parameters);
			}
	...
	for (size_t i = 0; i < kex_testcases_len; i++) {
		if (run_all || kex_testcases[i].run == 1) {
			...
			success = kex_test_correctness_wrapper(
			    rand, kex_testcases[i].alg_name, kex_testcases[i].seed,
			    kex_testcases[i].seed_len, kex_testcases[i].named_parameters,
			    num_iter, quiet);
		}
	...
	if (bench) {
		...
		for (size_t i = 0; i < kex_testcases_len; i++) {
			if (run_all || kex_testcases[i].run == 1) {
				kex_bench_wrapper(rand, kex_testcases[i].alg_name,
				                  kex_testcases[i].seed, kex_testcases[i].seed_len,
				                  kex_testcases[i].named_parameters, kex_bench_seconds);
			}
		}
		PRINT_TIMER_FOOTER
	}
