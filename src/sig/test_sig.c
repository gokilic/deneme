...
static OQS_STATUS sig_test_correctness(OQS_RAND *rand, enum OQS_SIG_algid algid,
                                       const int print) {
...
UNUSED static OQS_STATUS sig_test_correctness_wrapper(OQS_RAND *rand,
                                                      enum OQS_SIG_algid algid,
                                                      int iterations,
                                                      bool quiet) {
	OQS_STATUS ret;
...
UNUSED static OQS_STATUS
sig_bench_wrapper(OQS_RAND *rand, enum OQS_SIG_algid algid, const int seconds) {
	OQS_STATUS rc;
	...
	OQS_SIG_sign(s, priv, msg, msg_len, sig, &sig_len);
	TIME_OPERATION_SECONDS({
		OQS_SIG_verify(s, pub, msg, msg_len, sig, sig_len);
	},
	                       "verify", seconds);
...
int main(int argc, char **argv) {
	...
	bool bench = false;
	OQS_RAND *rand = NULL;
	size_t sig_testcases_len =
	    sizeof(sig_testcases) / sizeof(struct sig_testcase);
	...
	for (size_t i = 0; i < sig_testcases_len; i++) {
		if (run_all || sig_testcases[i].run == 1) {
			...
			success = sig_test_correctness_wrapper(rand, sig_testcases[i].algid,
			                                       num_iter, quiet);
		}
...
int main() {
	printf("No signature algorithm available. Make sure configure was run "
	       "properly; see Readme.md.\n");
	return 0;
}
