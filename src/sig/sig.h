...
struct OQS_SIG {
   ...
   * @param sig              The generated signature. Must be allocated by the
   * caller, or NULL to learn how much space is needed, as returned in sig_len.
   * @param sig_len          In: length of sig, out: length of the generated
   * signature.
   * @return                 OQS_SUCCESS on success, or OQS_ERROR on failure.
   */
	OQS_STATUS (*sign)
 ...
 * @param algid              The id of the signature algorithm to be
 * instantiated.
 * @return                   A new signature object on success, or NULL on
 * failure.
 */
 ...
 * @param priv               Pointer where the generated private key will be
 * stored. Caller
 *                           must have allocated s->priv_key_len bytes.
 * @param pub                Pointer where the generated public key will be
 * stored. Caller
 ...
 * @param s         Pointer to the signature object.
 * @param priv      Pointer to the signer's private key, of expected length
 * `s->priv_key_len` bytes.
 * @param msg       Pointer to the message to sign.
 * @param msg_len   Length of the message to sign `msg`.
 * @param sig       Pointer where the generated signature will be stored. Caller
 * must have allocated `s->max_sig_len` bytes.
 * @param sig_len   Pointer to the length of the generated signature.
 * @return          OQS_SUCCESS on success, or OQS_ERROR on failure
 */
OQS_STATUS
OQS_SIG_sign(const OQS_SIG *s, const uint8_t *priv, const uint8_t *msg,
             const size_t msg_len, uint8_t *sig, size_t *sig_len);

/**
 * Verifies a signature.
 * @param s         Pointer to the signature object.
 * @param pub       Pointer to the signer's public key, of expected length
 * `s->pub_key_len` bytes.
 ...
 */
OQS_STATUS
OQS_SIG_verify(const OQS_SIG *s, const uint8_t *pub, const uint8_t *msg,
               const size_t msg_len, const uint8_t *sig, const size_t sig_len);
