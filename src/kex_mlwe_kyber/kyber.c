...
static void keygen(unsigned char *pk, unsigned char *sk, OQS_RAND *rand) {
	...
	OQS_SHA3_shake128(sk + KYBER_SECRETKEYBYTES - 64, 32, pk,
	                  KYBER_PUBLICKEYBYTES);
	rand->rand_n(
	    rand, sk + KYBER_SECRETKEYBYTES - KYBER_SHAREDKEYBYTES,
	    KYBER_SHAREDKEYBYTES); /* Value z for pseudo-random output on reject */
}
...
static void sharedb(unsigned char *sharedkey, unsigned char *send,
	...
	OQS_SHA3_shake128(buf + 32, 32, received,
	                  KYBER_PUBLICKEYBYTES); /* Multitarget countermeasure for
	...
	OQS_SHA3_shake128(krq + 32, 32, send,
	                  KYBER_BYTES); /* overwrite coins in krq with h(c) */
	OQS_SHA3_shake128(sharedkey, 32, krq,
	                  64); /* hash concatenation of pre-k and h(c) to k */
...
static void shareda(unsigned char *sharedkey, const unsigned char *sk,
	...
	// shake128(buf+32, 32, pk, KYBER_PUBLICKEYBYTES); /* Multitarget
	// countermeasure for coins + contributory KEM */
	...
	OQS_SHA3_shake128(krq + 32, 32, received,
	                  KYBER_BYTES); /* overwrite coins in krq with h(c)  */

	cmov(krq, sk + KYBER_SECRETKEYBYTES - KYBER_SHAREDKEYBYTES,
	     KYBER_SHAREDKEYBYTES,
