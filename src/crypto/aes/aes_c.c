...
// This code is public domain, or any OSI-approved license, your choice. No
// warranty.
...
// Here are all the lookup tables for the row shifts, rcon, s-boxes, and galois
// field multiplications
static const byte shift_rows_table[] = {0, 5, 10, 15, 4, 9, 14, 3,
                                        8, 13, 2, 7, 12, 1, 6, 11};
static const byte shift_rows_table_inv[] = {0, 13, 10, 7, 4, 1, 14, 11,
                                            8, 5, 2, 15, 12, 9, 6, 3};
static const byte lookup_rcon[] = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10,
...
// Perform the core key schedule transform on 4 bytes, as part of the key
// expansion process
...
static void key_schedule_core(byte *a, int i) {
	...
	a[2] = a[3];
	a[3] = temp;
	sub_bytes(a, 4);        // Apply Rijndael's S-box on all four individual bytes in the
	                        // output word
	a[0] ^= lookup_rcon[i]; // On just the first (leftmost) byte of the output
	                        // word, perform the rcon operation with i
	                        // as the input, and exclusive or the rcon output with the first byte of the
	                        // output word
}
...
void oqs_aes128_load_schedule_c(const uint8_t *key, void **_schedule) {
	*_schedule = malloc(16 * 11);
	...
	memcpy(schedule, key, 16); // The first 16 bytes of the expanded key are
	                           // simply the encryption key

	while (bytes <
	       176) {                           // Until we have 176 bytes of expanded key, we do the following:
		memcpy(t, schedule + bytes - 4, 4); // We assign the value of the previous
		                                    // four bytes in the expanded key to t
		key_schedule_core(t, i);            // We perform the key schedule core on t, with i as
		                                    // the rcon iteration value
		i++;                                // We increment i by 1
		xor(t, schedule + bytes - 16, 4);   // We exclusive-or t with the four-byte
		                                    // block 16 bytes before the new expanded
		                                    // key.
		memcpy(schedule + bytes, t,
		       4);  // This becomes the next 4 bytes in the expanded key
		...
		for (j = 0; j < 3; j++) {
			memcpy(t, schedule + bytes - 4, 4); // We assign the value of the previous
			                                    // 4 bytes in the expanded key to t
			xor(t, schedule + bytes - 16,
			    4); // We exclusive-or t with the four-byte block n bytes before
			memcpy(schedule + bytes, t,
			       4);  // This becomes the next 4 bytes in the expanded key
			bytes += 4; // Keep track of how many expanded key bytes we've added
		}
...
void oqs_aes128_enc_c(const uint8_t *plaintext, const void *_schedule,
                      uint8_t *ciphertext) {
...
void oqs_aes128_dec_c(const uint8_t *ciphertext, const void *_schedule,
                      uint8_t *plaintext) {
	const uint8_t *schedule = (const uint8_t *) _schedule;
	int i; // To count the rounds
