...
#define OQS_RLWE_MSRLN16_NBITS_TO_NWORDS(nbits) \
	(((nbits) + (sizeof(digit_t) * 8) - 1) /    \
	 (sizeof(digit_t) *                         \
	  8)) // Conversion macro from number of bits to number of computer words
#define OQS_RLWE_MSRLN16_NBYTES_TO_NWORDS(nbytes) \
...
void oqs_rlwe_msrln16_NTT_CT_std2rev_12289(int32_t *a, const int32_t *psi_rev,
                                           unsigned int N);
void oqs_rlwe_msrln16_NTT_CT_std2rev_12289_asm(int32_t *a,
                                               const int32_t *psi_rev,
                                               unsigned int N);
...
void oqs_rlwe_msrln16_INTT_GS_rev2std_12289(int32_t *a,
                                            ...
                                            const int32_t Ninv, unsigned int N);
void oqs_rlwe_msrln16_INTT_GS_rev2std_12289_asm(int32_t *a,
                                                const int32_t *omegainv_rev,
                                                const int32_t omegainv1N_rev,
                                                const int32_t Ninv,
                                                unsigned int N);
...
void oqs_rlwe_msrln16_pmul_asm(int32_t *a, int32_t *b, int32_t *c,
                               unsigned int N);

// Component-wise multiplication and addition
void oqs_rlwe_msrln16_pmuladd(int32_t *a, int32_t *b, int32_t *c, int32_t *d,
                              unsigned int N);
void oqs_rlwe_msrln16_pmuladd_asm(int32_t *a, int32_t *b, int32_t *c,
...
void oqs_rlwe_msrln16_helprec_asm(const uint32_t *x, uint32_t *rvec,
                                  unsigned char *random_bits);

// Reconciliation
void oqs_rlwe_msrln16_Rec(const uint32_t *x, const uint32_t *rvec,
                          unsigned char *key);
void oqs_rlwe_msrln16_rec_asm(const uint32_t *x, const uint32_t *rvec,
                              unsigned char *key);
