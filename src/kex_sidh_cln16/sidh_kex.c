...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_EphemeralKeyGeneration_A(
    unsigned char *PrivateKeyA, unsigned char *PublicKeyA,
    PCurveIsogenyStruct CurveIsogeny,
    OQS_RAND *rand) { // Alice's ephemeral key-pair generation
	// It produces a private key PrivateKeyA and computes the public key
	// PublicKeyA.
	// The private key is an even integer in the range [2, oA-2], where oA =
	// 2^372.
	...
	unsigned int owords = NBITS_TO_NWORDS(CurveIsogeny->owordbits),
	             pwords = NBITS_TO_NWORDS(CurveIsogeny->pwordbits);
	oqs_sidh_cln16_point_basefield_t P;
	oqs_sidh_cln16_point_proj_t R, phiP = {0}, phiQ = {0}, phiD = {0},
	                               pts[SIDH_MAX_INT_POINTS_ALICE];
	oqs_sidh_cln16_publickey_t *PublicKey =
	    (oqs_sidh_cln16_publickey_t *) PublicKeyA;
	unsigned int i, row, m, index = 0, pts_index[SIDH_MAX_INT_POINTS_ALICE],
	                        npts = 0;
	oqs_sidh_cln16_f2elm_t coeff[5], A = {0}, C = {0}, Aout, Cout;
	SIDH_CRYPTO_STATUS Status = SIDH_CRYPTO_SUCCESS;

	if (PrivateKeyA == NULL || PublicKey == NULL ||
	    oqs_sidh_cln16_is_CurveIsogenyStruct_null(CurveIsogeny)) {
		return SIDH_CRYPTO_ERROR_INVALID_PARAMETER;
	}

	// Choose a random even number in the range [2, oA-2] as secret key for Alice
	Status = oqs_sidh_cln16_random_mod_order((digit_t *) PrivateKeyA, SIDH_ALICE,
	                                         CurveIsogeny, rand);
	...
	oqs_sidh_cln16_to_mont(
	    (digit_t *) CurveIsogeny->PA,
	    (digit_t *)
	        P); // Conversion of Alice's generators to Montgomery representation
	oqs_sidh_cln16_to_mont(((digit_t *) CurveIsogeny->PA) + NWORDS_FIELD,
	                       ((digit_t *) P) + NWORDS_FIELD);

	Status = oqs_sidh_cln16_secret_pt(P, (digit_t *) PrivateKeyA, SIDH_ALICE, R,
	                                  CurveIsogeny);
	if (Status != SIDH_CRYPTO_SUCCESS) {
		oqs_sidh_cln16_clear_words((void *) PrivateKeyA, owords);
		return Status;
	...
	oqs_sidh_cln16_copy_words(
	    (digit_t *) CurveIsogeny->PB, (digit_t *) phiP,
	    pwords); // Copy X-coordinates from Bob's public parameters, set Z <- 1
	oqs_sidh_cln16_fpcopy751((digit_t *) CurveIsogeny->Montgomery_one,
	                         (digit_t *) phiP->Z);
	oqs_sidh_cln16_to_mont((digit_t *) phiP, (digit_t *) phiP);
	oqs_sidh_cln16_copy_words((digit_t *) phiP, (digit_t *) phiQ,
	                          pwords); // QB = (-XPB:1)
	oqs_sidh_cln16_fpneg751(phiQ->X[0]);
	oqs_sidh_cln16_fpcopy751((digit_t *) CurveIsogeny->Montgomery_one,
	                         (digit_t *) phiQ->Z);
	oqs_sidh_cln16_distort_and_diff(phiP->X[0], phiD,
	                                CurveIsogeny); // DB = (x(QB-PB),z(QB-PB))

	oqs_sidh_cln16_fpcopy751(CurveIsogeny->A,
	                         A[0]); // Extracting curve parameters A and C
	...
	oqs_sidh_cln16_from_fp2mont(
	    ...
	    ((oqs_sidh_cln16_f2elm_t *)
	         PublicKey)[0]); // Converting back to standard representation
	oqs_sidh_cln16_from_fp2mont(phiQ->X,
	                            ((oqs_sidh_cln16_f2elm_t *) PublicKey)[1]);
	oqs_sidh_cln16_from_fp2mont(phiD->X,
	                            ((oqs_sidh_cln16_f2elm_t *) PublicKey)[2]);
	...
	oqs_sidh_cln16_clear_words((void *) pts,
	                           SIDH_MAX_INT_POINTS_ALICE * 2 * 2 * pwords);
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_EphemeralKeyGeneration_B(
    unsigned char *PrivateKeyB, unsigned char *PublicKeyB,
    PCurveIsogenyStruct CurveIsogeny,
    OQS_RAND *rand) { // Bob's ephemeral key-pair generation
	// It produces a private key PrivateKeyB and computes the public key
	// PublicKeyB.
	...
	oqs_sidh_cln16_point_basefield_t P;
	oqs_sidh_cln16_point_proj_t R, phiP = {0}, phiQ = {0}, phiD = {0},
	                               pts[SIDH_MAX_INT_POINTS_BOB];
	oqs_sidh_cln16_publickey_t *PublicKey =
	    (oqs_sidh_cln16_publickey_t *) PublicKeyB;
	unsigned int i, row, m, index = 0, pts_index[SIDH_MAX_INT_POINTS_BOB],
	                        npts = 0;
	oqs_sidh_cln16_f2elm_t A = {0}, C = {0};
	SIDH_CRYPTO_STATUS Status = SIDH_CRYPTO_SUCCESS;

	if (PrivateKeyB == NULL || PublicKey == NULL ||
	    oqs_sidh_cln16_is_CurveIsogenyStruct_null(CurveIsogeny)) {
		return SIDH_CRYPTO_ERROR_INVALID_PARAMETER;
	}

	// Choose a random number equivalent to 0 (mod 3) in the range [3, oB-3] as
	// secret key for Bob
	Status = oqs_sidh_cln16_random_mod_order((digit_t *) PrivateKeyB, SIDH_BOB,
	                                         CurveIsogeny, rand);
	if (Status != SIDH_CRYPTO_SUCCESS) {
	...
	oqs_sidh_cln16_to_mont(
	    (digit_t *) CurveIsogeny->PB,
	    (digit_t *)
	        P); // Conversion of Bob's generators to Montgomery representation
	oqs_sidh_cln16_to_mont(((digit_t *) CurveIsogeny->PB) + NWORDS_FIELD,
	                       ((digit_t *) P) + NWORDS_FIELD);

	Status = oqs_sidh_cln16_secret_pt(P, (digit_t *) PrivateKeyB, SIDH_BOB, R,
	                                  CurveIsogeny);
	if (Status != SIDH_CRYPTO_SUCCESS) {
		oqs_sidh_cln16_clear_words((void *) PrivateKeyB, owords);
		return Status;
	...
	oqs_sidh_cln16_copy_words(
	    (digit_t *) CurveIsogeny->PA, (digit_t *) phiP,
	    pwords); // Copy X-coordinates from Alice's public parameters, set Z <- 1
	oqs_sidh_cln16_fpcopy751((digit_t *) CurveIsogeny->Montgomery_one,
	                         (digit_t *) phiP->Z);
	oqs_sidh_cln16_to_mont(
	    (digit_t *) phiP,
	    (digit_t *) phiP); // Conversion to Montgomery representation
	oqs_sidh_cln16_copy_words((digit_t *) phiP, (digit_t *) phiQ,
	                          pwords); // QA = (-XPA:1)
	oqs_sidh_cln16_fpneg751(phiQ->X[0]);
	oqs_sidh_cln16_fpcopy751((digit_t *) CurveIsogeny->Montgomery_one,
	                         (digit_t *) phiQ->Z);
	oqs_sidh_cln16_distort_and_diff(phiP->X[0], phiD,
	                                CurveIsogeny); // DA = (x(QA-PA),z(QA-PA))
	...
	oqs_sidh_cln16_clear_words((void *) phiD, 2 * 2 * pwords);
	oqs_sidh_cln16_clear_words((void *) pts,
	                           SIDH_MAX_INT_POINTS_BOB * 2 * 2 * pwords);
	oqs_sidh_cln16_clear_words((void *) A, 2 * pwords);
	oqs_sidh_cln16_clear_words((void *) C, 2 * pwords);
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_EphemeralSecretAgreement_A(
    const unsigned char *PrivateKeyA, const unsigned char *PublicKeyB,
    unsigned char *SharedSecretA,
    PCurveIsogenyStruct
        CurveIsogeny) { // Alice's ephemeral shared secret computation
	// It produces a shared secret key SharedSecretA using her secret key
	// PrivateKeyA and Bob's public key PublicKeyB
	// Inputs: Alice's PrivateKeyA is an even integer in the range [2, oA-2],
	// where oA = 2^372.
	...
	if (PrivateKeyA == NULL || PublicKey == NULL || SharedSecretA == NULL ||
	    oqs_sidh_cln16_is_CurveIsogenyStruct_null(CurveIsogeny)) {
		return SIDH_CRYPTO_ERROR_INVALID_PARAMETER;
	}

	oqs_sidh_cln16_to_fp2mont(((oqs_sidh_cln16_f2elm_t *) PublicKey)[0],
	                          PKB[0]); // Extracting and converting Bob's public
	                                   // curve parameters to Montgomery
	                                   // representation
	...
	Status =
	    oqs_sidh_cln16_ladder_3_pt(PKB[0], PKB[1], PKB[2], (digit_t *) PrivateKeyA,
	                               SIDH_ALICE, R, A, CurveIsogeny);
	if (Status != SIDH_CRYPTO_SUCCESS) {
		return Status;
	}
	oqs_sidh_cln16_first_4_isog(R, A, A, C, CurveIsogeny);
	...
	oqs_sidh_cln16_from_fp2mont(
	    jinv, (oqs_sidh_cln16_felm_t *)
	              SharedSecretA); // Converting back to standard representation
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_EphemeralSecretAgreement_B(
    const unsigned char *PrivateKeyB, const unsigned char *PublicKeyA,
    unsigned char *SharedSecretB,
    PCurveIsogenyStruct
        CurveIsogeny) { // Bob's ephemeral shared secret computation
	// It produces a shared secret key SharedSecretB using his secret key
	// PrivateKeyB and Alice's public key PublicKeyA
	...
	if (PrivateKeyB == NULL || PublicKey == NULL || SharedSecretB == NULL ||
	    oqs_sidh_cln16_is_CurveIsogenyStruct_null(CurveIsogeny)) {
		return SIDH_CRYPTO_ERROR_INVALID_PARAMETER;
	}

	oqs_sidh_cln16_to_fp2mont(((oqs_sidh_cln16_f2elm_t *) PublicKey)[0],
	                          PKA[0]); // Extracting and converting Alice's public
	                                   // curve parameters to Montgomery
	                                   // representation
	...
	Status =
	    oqs_sidh_cln16_ladder_3_pt(PKA[0], PKA[1], PKA[2], (digit_t *) PrivateKeyB,
	                               SIDH_BOB, R, A, CurveIsogeny);
	if (Status != SIDH_CRYPTO_SUCCESS) {
		return Status;
	}
	...
	oqs_sidh_cln16_from_fp2mont(
	    jinv, (oqs_sidh_cln16_felm_t *)
	              SharedSecretB); // Converting back to standard representation
...
void oqs_sidh_cln16_PublicKeyCompression_A(
    const unsigned char *PublicKeyA, unsigned char *CompressedPKA,
    PCurveIsogenyStruct CurveIsogeny) { // Alice's public key compression
	...
	digit_t a0[SIDH_NWORDS_ORDER], b0[SIDH_NWORDS_ORDER], a1[SIDH_NWORDS_ORDER],
	...
	oqs_sidh_cln16_ph3(phiP, phiQ, R1, R2, A, (uint64_t *) a0, (uint64_t *) b0,
	                   (uint64_t *) a1, (uint64_t *) b1, CurveIsogeny);

	bit = oqs_sidh_cln16_mod3(a0);
	oqs_sidh_cln16_to_Montgomery_mod_order(
	    a0, a0, CurveIsogeny->Border, (digit_t *) &Montgomery_rprime,
	    (digit_t *) &Montgomery_Rprime); // Converting to Montgomery representation
	oqs_sidh_cln16_to_Montgomery_mod_order(a1, a1, CurveIsogeny->Border,
	                                       (digit_t *) &Montgomery_rprime,
	                                       (digit_t *) &Montgomery_Rprime);
	oqs_sidh_cln16_to_Montgomery_mod_order(b0, b0, CurveIsogeny->Border,
	                                       (digit_t *) &Montgomery_rprime,
	                                       (digit_t *) &Montgomery_Rprime);
	oqs_sidh_cln16_to_Montgomery_mod_order(b1, b1, CurveIsogeny->Border,
	                                       (digit_t *) &Montgomery_rprime,
	                                       (digit_t *) &Montgomery_Rprime);
	...
	if (bit !=
		...
		oqs_sidh_cln16_Montgomery_inversion_mod_order_bingcd(
		    a0, inv, CurveIsogeny->Border, (digit_t *) &Montgomery_rprime,
		    (digit_t *) &Montgomery_Rprime);
		oqs_sidh_cln16_Montgomery_multiply_mod_order(
		    b0, inv, &comp[0], CurveIsogeny->Border, (digit_t *) &Montgomery_rprime);
		oqs_sidh_cln16_Montgomery_multiply_mod_order(
		    a1, inv, &comp[SIDH_NWORDS_ORDER], CurveIsogeny->Border,
		    (digit_t *) &Montgomery_rprime);
		oqs_sidh_cln16_Montgomery_multiply_mod_order(
		    b1, inv, &comp[2 * SIDH_NWORDS_ORDER], CurveIsogeny->Border,
		    (digit_t *) &Montgomery_rprime);
		...
		oqs_sidh_cln16_Montgomery_inversion_mod_order_bingcd(
		    b0, inv, CurveIsogeny->Border, (digit_t *) &Montgomery_rprime,
		    (digit_t *) &Montgomery_Rprime);
		oqs_sidh_cln16_Montgomery_multiply_mod_order(
		    a0, inv, &comp[0], CurveIsogeny->Border, (digit_t *) &Montgomery_rprime);
		oqs_sidh_cln16_Montgomery_multiply_mod_order(
		    a1, inv, &comp[SIDH_NWORDS_ORDER], CurveIsogeny->Border,
		    (digit_t *) &Montgomery_rprime);
		oqs_sidh_cln16_Montgomery_multiply_mod_order(
...
void oqs_sidh_cln16_PublicKeyADecompression_B(
    ...
    PCurveIsogenyStruct CurveIsogeny) { // Alice's public key value
	                                    // decompression computed by Bob
	...
	// Output: a point point_R in coordinates (X:Z) and the curve parameter
	// param_A in GF(p751^2). Outputs are stored in Montgomery representation.
	// CurveIsogeny must be set up in advance using SIDH_curve_initialize().
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_EphemeralSecretAgreement_Compression_A(
    ...
    PCurveIsogenyStruct
        CurveIsogeny) { // Alice's ephemeral shared secret computation
	// It produces a shared secret key SharedSecretA using her secret key
	// PrivateKeyA and Bob's decompressed data point_R and param_A
	// Inputs: Alice's PrivateKeyA is an even integer in the range [2, oA-2],
	// where oA = 2^372.
	//         Bob's decompressed data consists of point_R in (X:Z) coordinates
	...
	oqs_sidh_cln16_fp2copy751((((oqs_sidh_cln16_point_proj_t *) point_R)[0])->X,
	                          R->X);
	oqs_sidh_cln16_fp2copy751((((oqs_sidh_cln16_point_proj_t *) point_R)[0])->Z,
	                          R->Z);
	oqs_sidh_cln16_fpcopy751(CurveIsogeny->C, C[0]);
	oqs_sidh_cln16_to_mont(C[0], C[0]);
	oqs_sidh_cln16_first_4_isog(R, (oqs_sidh_cln16_felm_t *) param_A, A, C,
	                            CurveIsogeny);
...
void oqs_sidh_cln16_PublicKeyCompression_B(
    const unsigned char *PublicKeyB, unsigned char *CompressedPKB,
    PCurveIsogenyStruct CurveIsogeny) { // Bob's public key compression
	...
	oqs_sidh_cln16_f2elm_t A, vec[4], Zinv[4];
	digit_t a0[SIDH_NWORDS_ORDER], b0[SIDH_NWORDS_ORDER], a1[SIDH_NWORDS_ORDER],
	    b1[SIDH_NWORDS_ORDER], tmp[2 * SIDH_NWORDS_ORDER], mask = (digit_t)(-1);

	mask >>= (CurveIsogeny->owordbits - CurveIsogeny->oAbits);
	oqs_sidh_cln16_to_fp2mont(
	    ((oqs_sidh_cln16_f2elm_t *) PublicKeyB)[0],
	    ((oqs_sidh_cln16_f2elm_t
	          *) &PK)[0]); // Converting to Montgomery representation
	oqs_sidh_cln16_to_fp2mont(((oqs_sidh_cln16_f2elm_t *) PublicKeyB)[1],
	                          ((oqs_sidh_cln16_f2elm_t *) &PK)[1]);
	oqs_sidh_cln16_to_fp2mont(((oqs_sidh_cln16_f2elm_t *) PublicKeyB)[2],
	                          ((oqs_sidh_cln16_f2elm_t *) &PK)[2]);
	...
	oqs_sidh_cln16_ph2(phiP, phiQ, R1, R2, A, (uint64_t *) a0, (uint64_t *) b0,
	                   (uint64_t *) a1, (uint64_t *) b1, CurveIsogeny);
...
void oqs_sidh_cln16_PublicKeyBDecompression_A(
    ...
    unsigned char *point_R, unsigned char *param_A,
    PCurveIsogenyStruct CurveIsogeny) { // Bob's public key value decompression
	                                    // computed by Alice
	...
	if (bit == 0) {
		oqs_sidh_cln16_multiply((digit_t *) SecretKeyA, &comp[SIDH_NWORDS_ORDER],
		...
		oqs_sidh_cln16_multiply((digit_t *) SecretKeyA, &comp[2 * SIDH_NWORDS_ORDER],
		                        tmp1, SIDH_NWORDS_ORDER);
