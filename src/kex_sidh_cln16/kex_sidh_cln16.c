...
extern bool
oqs_sidh_cln16_is_CurveIsogenyStruct_null(PCurveIsogenyStruct pCurveIsogeny);
...
OQS_KEX *OQS_KEX_sidh_cln16_new(OQS_RAND *rand, const char *named_parameters) {
	...
	PCurveIsogenyStruct curveIsogeny =
	    oqs_sidh_cln16_curve_allocate(&CurveIsogeny_SIDHp751);
	...
	if (oqs_sidh_cln16_curve_initialize(curveIsogeny, &CurveIsogeny_SIDHp751) !=
	    SIDH_CRYPTO_SUCCESS) {
		free(k);
		oqs_sidh_cln16_curve_free(curveIsogeny);
		return NULL;
	...
	k->ctx = curveIsogeny;
	k->method_name =
	    compressed ? strdup("SIDH CLN16 compressed") : strdup("SIDH CLN16");
...
int OQS_KEX_sidh_cln16_alice_0(OQS_KEX *k, void **alice_priv,
	...
	*alice_priv = malloc(SIDH_SECRETKEY_LEN);
	...
	if (oqs_sidh_cln16_EphemeralKeyGeneration_A(
	        (unsigned char *) *alice_priv, (unsigned char *) alice_tmp_pub, k->ctx,
	        k->rand) != SIDH_CRYPTO_SUCCESS) {
		goto err;
	}

	if (compressed) {
		// compress Alice's public key
		oqs_sidh_cln16_PublicKeyCompression_A(alice_tmp_pub,
		                                      (unsigned char *) *alice_msg, k->ctx);
...
int OQS_KEX_sidh_cln16_bob(OQS_KEX *k, const uint8_t *alice_msg,
	...
	*key = malloc(SIDH_SHAREDKEY_LEN);
	...
	if (oqs_sidh_cln16_EphemeralKeyGeneration_B(
	        (unsigned char *) bob_priv, (unsigned char *) bob_tmp_pub, k->ctx,
	        k->rand) != SIDH_CRYPTO_SUCCESS) {
		goto err;
	}
	...
	if (compressed) {
		...
		oqs_sidh_cln16_PublicKeyCompression_B(bob_tmp_pub,
		                                      (unsigned char *) *bob_msg, k->ctx);
		*bob_msg_len = SIDH_COMPRESSED_PUBKEY_LEN;
		// decompress Alice's public key
		oqs_sidh_cln16_PublicKeyADecompression_B(
		    (unsigned char *) bob_priv, (unsigned char *) alice_msg, R, A, k->ctx);
		// compute Bob's shared secret
		if (oqs_sidh_cln16_EphemeralSecretAgreement_Compression_B(
		        (unsigned char *) bob_priv, R, A, (unsigned char *) *key, k->ctx) !=
		    SIDH_CRYPTO_SUCCESS) {
			goto err;
		}
	} else {
		*bob_msg_len = SIDH_PUBKEY_LEN;
		...
		// compute Bob's shared secret
		if (oqs_sidh_cln16_EphemeralSecretAgreement_B(
		        (unsigned char *) bob_priv, (unsigned char *) alice_msg,
		        (unsigned char *) *key, k->ctx) != SIDH_CRYPTO_SUCCESS) {
			goto err;
		}
	}
	...
	*key_len = SIDH_SHAREDKEY_LEN;
	...
	if (compressed) {
		...
		oqs_sidh_cln16_PublicKeyBDecompression_A(
		    (unsigned char *) alice_priv, (unsigned char *) bob_msg, R, A, k->ctx);
		if (oqs_sidh_cln16_EphemeralSecretAgreement_Compression_A(
		        (unsigned char *) alice_priv, R, A, (unsigned char *) *key, k->ctx) !=
		    SIDH_CRYPTO_SUCCESS) {
			goto err;
		}
	} else {
		...
		if (oqs_sidh_cln16_EphemeralSecretAgreement_A(
		        (unsigned char *) alice_priv, (unsigned char *) bob_msg,
		        (unsigned char *) *key, k->ctx) != SIDH_CRYPTO_SUCCESS) {
			goto err;
		}
	}
