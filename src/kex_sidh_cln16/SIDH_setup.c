...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_curve_initialize(
    PCurveIsogenyStruct pCurveIsogeny,
    PCurveIsogenyStaticData pCurveIsogenyData) { // Initialize curve isogeny
	                                             // structure pCurveIsogeny with
	...
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->prime,
	                          pCurveIsogeny->prime, pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->A, pCurveIsogeny->A,
	                          pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->C, pCurveIsogeny->C,
	                          pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->Aorder,
	                          pCurveIsogeny->Aorder, owords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->Border,
	                          pCurveIsogeny->Border, owords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->PA, pCurveIsogeny->PA,
	                          2 * pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->PB, pCurveIsogeny->PB,
	                          2 * pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->BigMont_order,
	                          pCurveIsogeny->BigMont_order, pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->Montgomery_R2,
	                          pCurveIsogeny->Montgomery_R2, pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->Montgomery_pp,
	                          pCurveIsogeny->Montgomery_pp, pwords);
	oqs_sidh_cln16_copy_words((digit_t *) pCurveIsogenyData->Montgomery_one,
	                          pCurveIsogeny->Montgomery_one, pwords);

	return SIDH_CRYPTO_SUCCESS;
}

PCurveIsogenyStruct oqs_sidh_cln16_curve_allocate(
    PCurveIsogenyStaticData CurveData) { // Dynamic allocation of memory for
	                                     // curve isogeny structure.
...
void oqs_sidh_cln16_curve_free(
    PCurveIsogenyStruct
        pCurveIsogeny) { // Free memory for curve isogeny structure
...
bool oqs_sidh_cln16_is_CurveIsogenyStruct_null(
    PCurveIsogenyStruct
        pCurveIsogeny) { // Check if curve isogeny structure is NULL

	if (pCurveIsogeny == NULL || pCurveIsogeny->prime == NULL ||
	    pCurveIsogeny->A == NULL || pCurveIsogeny->C == NULL ||
	    pCurveIsogeny->Aorder == NULL || pCurveIsogeny->Border == NULL ||
	    pCurveIsogeny->PA == NULL || pCurveIsogeny->PB == NULL ||
	    pCurveIsogeny->BigMont_order == NULL ||
	    pCurveIsogeny->Montgomery_R2 == NULL ||
	    pCurveIsogeny->Montgomery_pp == NULL ||
...
const uint64_t Border_div3[SIDH_NWORDS_ORDER] = {
    0xEDCD718A828384F9, 0x733B35BFD4427A14, 0xF88229CF94D7CF38,
    0x63C56C990C7C2AD6, 0xB858A87E8F4222C7, 0x254C9C6B525EAF5};

SIDH_CRYPTO_STATUS oqs_sidh_cln16_random_mod_order(
    digit_t *random_digits, unsigned int AliceOrBob,
    PCurveIsogenyStruct pCurveIsogeny,
    OQS_RAND *rand) { // Output random values in the range [1, order-1] in
	                  // little endian format that can be used as private keys.
	// It makes requests of random values with length "oAbits" (when AliceOrBob =
	// 0) or "oBbits" (when AliceOrBob = 1) to the "random_bytes" function.
	// The process repeats until random value is in [0, Aorder-2]  ([0, Border-2],
	// resp.).
	// If successful, the output is given in "random_digits" in the range [1,
	// Aorder-1] ([1, Border-1], resp.).
	...
	if (random_digits == NULL ||
	    oqs_sidh_cln16_is_CurveIsogenyStruct_null(pCurveIsogeny) ||
	    AliceOrBob > 1) {
		return SIDH_CRYPTO_ERROR_INVALID_PARAMETER;
	}
	...
	if (AliceOrBob == SIDH_ALICE) {
		nbytes = (pCurveIsogeny->oAbits + 7) /
		         8; // Number of random bytes to be requested
		...
		oqs_sidh_cln16_mp_sub((digit_t *) Border_div3, t1, order2,
		                      nwords); // order2 = order/3-2
	}

	do {
		ntry++;
		if (ntry >
		    100) { // Max. 100 iterations to obtain random value in [0, order-2]
	...
	oqs_sidh_cln16_mp_shiftl1(random_digits,
	                          nwords); // Alice's output in the range [2, order-2]
	if (AliceOrBob == SIDH_BOB) {
		oqs_sidh_cln16_mp_add(random_digits, t1, random_digits,
		                      nwords); // Bob's output in the range [3, order-3]
	}
...
SIDH_CRYPTO_STATUS oqs_sidh_cln16_random_BigMont_mod_order(
    digit_t *random_digits, PCurveIsogenyStruct pCurveIsogeny,
    OQS_RAND *rand) { // Output random values in the range [1, BigMont_order-1]
	                  // in little endian format that can be used as private
	                  // keys to compute scalar multiplications
	                  // using the elliptic curve BigMont.
	                  // It makes requests of random values with length
	// "BIGMONT_SIDH_SIDH_NBITS_ORDER" to the "random_bytes" function.
	...
	unsigned int ntry = 0, nbytes = (BIGMONT_SIDH_SIDH_NBITS_ORDER + 7) / 8,
	             nwords = NBITS_TO_NWORDS(BIGMONT_SIDH_SIDH_NBITS_ORDER);
	digit_t t1[BIGMONT_MAXWORDS_ORDER] = {0},
	        order2[BIGMONT_MAXWORDS_ORDER] = {0};
	unsigned char mask;
	SIDH_CRYPTO_STATUS Status = SIDH_CRYPTO_SUCCESS;
	...
	oqs_sidh_cln16_mp_sub(pCurveIsogeny->BigMont_order, t1, order2,
	                      nwords);       // order2 = order-2
	...
	t1[0] = 1;
	oqs_sidh_cln16_mp_add(random_digits, t1, random_digits,
	                      nwords); // Output in the range [1, order-1]

	return Status;
}

void oqs_sidh_cln16_clear_words(
    void *mem, digit_t nwords) { // Clear digits from memory. "nwords" indicates
	                             // the number of digits to be zeroed.
