...
int oqs_sidh_iqc_ref_public_key_is_valid(const public_key_t public_key,
	...
	if (!oqs_sidh_iqc_ref_public_key_check_order(public_key->P, public_key->E,
	                                             params))
		return 0;

	if (!oqs_sidh_iqc_ref_public_key_check_order(public_key->Q, public_key->E,
	                                             params))
		return 0;
...
int oqs_sidh_iqc_ref_public_key_check_dependency(const public_key_t public_key,
	...
	int result = 0;
	oqs_sidh_iqc_ref_elliptic_curve_prime_power_dlp(
	    x, public_key->P, public_key->Q, public_key->E, params->l, params->e);

	if (mpz_cmp_si(x, -1) == 0) {
		oqs_sidh_iqc_ref_elliptic_curve_prime_power_dlp(
		    x, public_key->Q, public_key->P, public_key->E, params->l, params->e);
		if (mpz_cmp_si(x, -1) == 0)
			result = 1;
	}
